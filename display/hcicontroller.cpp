...
void HCIInstance::senderEntry(HANDLE hPipe) {
    ...
    while (!shouldStop) {
        ...
        case HCIMESSAGEINTYPE_POSITION:
        {
            ...
            nowtime = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count();
            ...
            *(int64_t*)(buf + 11) = nowtime2;
            ...
            int cnt = mParent->core->GetPositionRawData(mouseIdx, buf + 1);
            if (cnt > 0) {
                DWORD length = 0;
...
#endif
#ifdef USE_MOUSECORE_SP
             int cnt = mParent->core->GetAngleRawData(mouseIdx, buf + 1);
             if (cnt > 0) {
                 DWORD length = 0;
                 int currentState = messageType.load();
                 if (nowState == currentState) {
    ...
    DisconnectNamedPipe(hPipe);
    CloseHandle(hPipe);
    std::cout << "HCIInstance senderEntry thread Stopped.\n";
}
...
void HCIInstance::receiverEntry(HANDLE hPipe) {
  ...
  while (!shouldStop) {
    ...
    switch (thisReport.type) {
      ...
      case HCIMESSAGEOUTTYPE_ANGLE:
        {
          float angle = *(float*)&(buf[2]);
          float ff = (540 - angle) / 180.0f * 3.1415926535897f;
          thisReport.param1 = *(uint32_t *) &ff;
          thisReport.param2 = int32_t(angle);
        }
        break;
  ...
  CloseHandle(hPipe);
  std::cout << "HCIInstance receiverEntry thread Stopped.\n";
}
