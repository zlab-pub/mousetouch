...
// Reading "size" bytes from serial port;
// returns read bytes as string;
std::string SerialPort::readSerialPort(unsigned int size)
{
    DWORD bytesRead{};
    unsigned int toRead = 0;
    ...
    if (ReadFile(this->handler, (void*)buffer, toRead, &bytesRead, NULL))
    {
        str.append(reinterpret_cast<const char*>(buffer), bytesRead);
    }
    delete[] buffer;
    return str;
}
...
bool SerialPort::writeSerialPort(char data)
{
    ...
    if (!WriteFile(this->handler, &data, 1, &bytesSend, 0))
    {
        ClearCommError(this->handler, &this->errors, &this->status);
        return false;
    }
